...
async function handler(req: NextApiRequest, res: NextApiResponse) {
  ...
  switch (req.method) {
    ...
    case 'POST':
      return createSatelliteData();
    default:
      return res.status(405).json({ message: `Method ${req.method} not allowed` });
  }

  async function getSatelliteData() {
    const data = await prisma.satellite_data
      .withAuthorization({
        roqUserId,
        tenantId: user.tenantId,
        roles: user.roles,
      })
      .findMany(convertQueryToPrismaUtil(req.query, 'satellite_data'));
  ...
  async function createSatelliteData() {
    ...
    const data = await prisma.satellite_data.create({
      data: body,
    });
    return res.status(200).json(data);
  }
