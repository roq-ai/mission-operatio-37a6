...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['ground-station-engineer', 'mission-analyst', 'satellite-operator'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<SatelliteOperatorInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/satellite-operators` : null),
    () => getSatelliteOperators({ tenant_id: session?.user?.tenantId }),
  );
  loading = isLoading;
  ...
  const LinkItems: Array<LinkItemProps> = [
...

    {
      ...
      path: isTenantUser ? `/satellite-operators/view/${data?.[0]?.id}` : '/satellite-operators',
